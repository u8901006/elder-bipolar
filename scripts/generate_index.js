#!/usr/bin/env node
import { readFileSync, writeFileSync, readdirSync } from "fs";

const docsDir = "docs";
const files = readdirSync(docsDir)
  .filter((f) => f.startsWith("oabd-") && f.endsWith(".html"))
  .sort()
  .reverse();

const weekdays = ["一", "二", "三", "四", "五", "六", "日"];

let links = "";
for (const name of files.slice(0, 60)) {
  const dateStr = name.replace("oabd-", "").replace(".html", "");
  let dateDisplay = dateStr;
  let weekday = "";
  if (/^\d{4}-\d{2}-\d{2}$/.test(dateStr)) {
    const [y, m, d] = dateStr.split("-");
    dateDisplay = `${y}年${parseInt(m)}月${parseInt(d)}日`;
    try {
      const dt = new Date(`${dateStr}T00:00:00+08:00`);
      weekday = weekdays[dt.getDay()];
    } catch {}
  }
  links += `      <li><a href="${name}">${dateDisplay}${weekday ? `（週${weekday}）` : ""}</a></li>\n`;
}

const total = files.length;

const html = `<!DOCTYPE html>
<html lang="zh-TW">
<head>
<meta charset="UTF-8"/>
<meta name="viewport" content="width=device-width, initial-scale=1.0"/>
<title>老年躁鬱症研究日報 · OABD Daily Literature Report</title>
<meta name="description" content="老年躁鬱症 (Older-Age Bipolar Disorder) 每日研究文獻日報，由 AI 自動彙整 PubMed 最新論文"/>
<style>
  :root { --bg: #f6f1e8; --surface: #fffaf2; --line: #d8c5ab; --text: #2b2118; --muted: #766453; --accent: #8c4f2b; --accent-soft: #ead2bf; }
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  body { background: radial-gradient(circle at top, #fff6ea 0, var(--bg) 55%, #ead8c6 100%); color: var(--text); font-family: "Noto Sans TC", "PingFang TC", "Helvetica Neue", Arial, sans-serif; min-height: 100vh; }
  .container { position: relative; z-index: 1; max-width: 640px; margin: 0 auto; padding: 80px 24px; }
  .logo { font-size: 48px; text-align: center; margin-bottom: 16px; }
  h1 { text-align: center; font-size: 24px; color: var(--text); margin-bottom: 8px; }
  .subtitle { text-align: center; color: var(--accent); font-size: 14px; margin-bottom: 8px; }
  .desc { text-align: center; color: var(--muted); font-size: 13px; margin-bottom: 48px; line-height: 1.6; max-width: 480px; margin-left: auto; margin-right: auto; }
  .count { text-align: center; color: var(--muted); font-size: 13px; margin-bottom: 32px; }
  ul { list-style: none; }
  li { margin-bottom: 8px; }
  a { color: var(--text); text-decoration: none; display: block; padding: 14px 20px; background: var(--surface); border: 1px solid var(--line); border-radius: 12px; transition: all 0.2s; font-size: 15px; }
  a:hover { background: var(--accent-soft); border-color: var(--accent); transform: translateX(4px); }
  .links-row { display: flex; gap: 12px; justify-content: center; margin-top: 40px; flex-wrap: wrap; }
  .links-row a { display: inline-flex; align-items: center; gap: 8px; padding: 10px 18px; font-size: 13px; font-weight: 600; }
  footer { margin-top: 56px; text-align: center; font-size: 12px; color: var(--muted); }
  footer a { display: inline; padding: 0; background: none; border: none; color: var(--muted); }
  footer a:hover { color: var(--accent); }
</style>
</head>
<body>
<div class="container">
  <div class="logo">&#x1F9E0;</div>
  <h1>老年躁鬱症研究日報</h1>
  <p class="subtitle">OABD Daily Literature Report · 每日自動更新</p>
  <p class="desc">每日自動從 PubMed 彙整最新老年躁鬱症 (Older-Age Bipolar Disorder) 研究文獻，由 AI 分析摘要、分類、PICO 解析，產生繁體中文日報。</p>
  <p class="count">共 ${total} 期日報</p>
  <ul>
${links}  </ul>
  <div class="links-row">
    <a href="https://www.leepsyclinic.com/" target="_blank" rel="noopener">&#x1F3E5; 李政洋身心診所</a>
    <a href="https://blog.leepsyclinic.com/" target="_blank" rel="noopener">&#x1F4E8; 訂閱電子報</a>
    <a href="https://buymeacoffee.com/CYlee" target="_blank" rel="noopener">&#x2615; Buy Me a Coffee</a>
  </div>
  <footer>
    <p>Powered by PubMed + Zhipu AI · <a href="https://github.com/u8901006/elder-bipolar">GitHub</a></p>
  </footer>
</div>
</body>
</html>`;

writeFileSync(`${docsDir}/index.html`, html, "utf8");
console.log(`Index page generated (${total} reports)`);
